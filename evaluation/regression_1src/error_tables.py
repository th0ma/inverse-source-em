"""
Error table evaluation for 1‑source regression model.

Computes full arrays of:
- xy_abs
- rho_abs
- phi_abs
- I_abs
...
And returns:
- full arrays (as lists)
- summary statistics (mean, p50, p90, p99)
...
def evaluate(model, X, Y, device="cpu", batch_size=256):
    """
    Compute full error arrays and summary statistics for the 1‑source regression model.
    """
    ...
    arrays = {
        "xy_abs": xy_all.tolist(),
        "rho_abs": rho_all.tolist(),
        "phi_abs": phi_all.tolist(),
        "I_abs": I_all.tolist(),
        "rho_rel": rho_rel.tolist(),
        "I_rel": I_rel.tolist(),
    }
    ...
    status = "passed"
    if any(_is_bad(v) for v in summaries.values()):
        status = "failed"
